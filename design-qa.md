...
- Interaction evidence: desktop feature dropdown exposes three destinations; mobile navigation toggles `aria-expanded`; annual/monthly pricing changes Essential from $14 to $18; FAQ state changes and reveals the selected answer; demo CTA reaches `/workspace` with populated demo state.
...
- Interaction evidence: trigger toggled `aria-expanded`; the named menu became visible; clicking the holdings filter outside dismissed it; Manage portfolios navigated to `/workspace/settings`, removed the menu and preserved return navigation.
...
- Functional evidence: physically selected, advanced, reversed and re-advanced through steps; toggled a multi-select asset on and off; selected the final referral answer; and completed the flow to `/workspace?welcome=1` with meaningful portfolio DOM and zero browser warnings/errors.
...
- Production evidence: `https://masterdeck.app/workspace/holdings/BHP` rendered BHP with five metrics and the Price chart; AAPL rendered its own identity and `Recorded trades` history; live Trades and Notes states worked; desktop and 390 x 844 mobile checks reported no page overflow and no browser errors.
