...
The asset detail page requests the latest 14 trading sessions from `/api/market-movement`. It loads verified price history and, when available, matches dated articles from free public RSS feeds to those sessions. Each matched entry keeps its original source link; unmatched sessions remain price-only rather than receiving invented explanations. No market-news API key is required.
