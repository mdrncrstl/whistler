...
function watchConsole(page, label) {
  page.on('console', (message) => {
    if (['error', 'warning'].includes(message.type()) && !message.text().includes('Reduced Motion enabled on your device')) consoleIssues.push({ page: label, type: message.type(), text: message.text() })
  })
...
try {
  ...
  await desktop.addInitScript(() => sessionStorage.setItem('masterdeck-demo', 'true'))
  ...
  await reduced.addInitScript(() => sessionStorage.setItem('masterdeck-demo', 'true'))
