...
try {
  ...
  await page.waitForURL('**/deck/referrals')
  await page.getByRole('heading', { name: 'Refer & earn' }).waitFor()
  check('account menu referral navigation', page.url().endsWith('/deck/referrals'))
  ...
  await mobilePage.goto(`${baseUrl}/deck/referrals`, { waitUntil: 'networkidle' })
  await mobilePage.getByRole('heading', { name: 'Refer & earn' }).waitFor()
  check('mobile route', mobilePage.url().endsWith('/deck/referrals'))
