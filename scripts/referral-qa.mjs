...
try {
  ...
  await page.waitForURL('**/workspace/referrals')
  await page.getByRole('heading', { name: 'Refer & earn' }).waitFor()
  check('account menu referral navigation', page.url().endsWith('/workspace/referrals'))
  ...
  await mobilePage.goto(`${baseUrl}/workspace/referrals`, { waitUntil: 'networkidle' })
  await mobilePage.getByRole('heading', { name: 'Refer & earn' }).waitFor()
  check('mobile route', mobilePage.url().endsWith('/workspace/referrals'))
