...
export function Landing({ onDemo, signedIn = false, onOpenApp }: LandingProps) {
  ...
  return <div className="marketing-page">
    ...
    <main id="top">
      ...
      <section id="pricing" className="marketing-section marketing-container"><SectionIntro eyebrow="Straightforward pricing" title="Full portfolio intelligence, priced lower." copy="Every plan is exactly 30% below the comparable Navexa tier. All prices are AUD." /><div className="marketing-billing-toggle" aria-label="Billing period"><button aria-pressed={!annual} className={!annual ? 'active' : ''} onClick={() => setAnnual(false)}>Monthly</button><button aria-pressed={annual} className={annual ? 'active' : ''} onClick={() => setAnnual(true)}>Annual <span>save 26%</span></button></div><div className="marketing-pricing-grid">{billingPlans.map(plan => <article key={plan.id} className={plan.featured ? 'featured' : ''}>{plan.featured && <span className="marketing-plan-badge">Most popular</span>}<h3>{plan.name}</h3><p>Up to {plan.portfolios} {plan.portfolios === 1 ? 'portfolio' : 'portfolios'}</p><div className="marketing-price"><strong>${formatAud(annual ? plan.annual : plan.monthly)}</strong><span>AUD / month</span></div><small>{annual ? `$${formatAud(plan.annualTotal)} billed annually · save ${annualSavingsPercent(plan)}%` : 'Billed monthly. Cancel anytime.'}</small><button className="marketing-button" onClick={beginGoogleSignIn}>Choose {plan.name}</button><ul>{plan.features.map(item => <li key={item}><Check />{item}</li>)}</ul></article>)}</div></section>
