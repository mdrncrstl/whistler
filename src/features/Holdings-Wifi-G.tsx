      ...
      <Card className="data-card">
        <div className="data-toolbar">
          ...
          <div className="toolbar-filters">
            <Select value={broker} onChange={(event) => setBroker(event.target.value)} aria-label="Filter holdings by broker"><option value="all">All brokers</option><option value="ibkr">IBKR</option><option value="superhero">Superhero</option><option value="snaptrade">Connected broker</option></Select>
        ...
        {rows.length ? <div className="table-scroll"><table className="holdings-table"><thead><tr><th>Asset</th><th>Broker / account</th><th className="numeric">Quantity</th><th className="numeric">Price</th><th className="numeric">Market value</th><th className="numeric">Cost</th><th className="numeric">Unrealised</th><th className="numeric">Today</th></tr></thead><tbody>{rows.map((item) => <HoldingNavigationRow symbol={item.symbol} key={`${item.provider}-${item.account_name}-${item.symbol}`}><td><span className="asset-cell"><HoldingLogo symbol={item.symbol} market={item.market} assetClass={item.asset_class} size={29} /><span><strong>{item.symbol}</strong><small>{item.name || item.symbol}</small></span></span></td><td><Badge tone={item.provider === 'ibkr' ? 'purple' : 'success'}>{item.provider === 'ibkr' ? 'IBKR' : item.provider === 'superhero' ? 'Superhero' : 'Connected broker'}</Badge><small className="account-subline">{item.account_name}</small></td><td className="numeric">{number(item.quantity, 4)}</td><td className="numeric">{money(item.current_price, item.currency, 2)}<small className="currency-subline">{item.currency}</small></td><td className="numeric private-value"><strong>{money(item.value_aud)}</strong><small>{summary.invested ? (item.value_aud / summary.invested * 100).toFixed(1) : '0'}% weight</small></td><td className="numeric private-value">{money(item.cost_aud)}</td><td className={`numeric private-value ${item.unrealised_gain_aud >= 0 ? 'positive' : 'negative'}`}><strong>{money(item.unrealised_gain_aud)}</strong><small>{percent(item.return_pct)}</small></td><td className={`numeric private-value ${item.day_change_aud >= 0 ? 'positive' : 'negative'}`}>{money(item.day_change_aud)}</td></HoldingNavigationRow>)}</tbody></table></div> : <EmptyState icon={bundle.holdings.length ? SearchX : BriefcaseBusiness} title={bundle.holdings.length ? 'No holdings match these filters' : 'No holdings imported'} description={bundle.holdings.length ? 'Clear the search or choose a different account.' : 'Import records from a compatible broker export or supported direct feed to populate this table.'} />}
