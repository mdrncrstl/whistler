...
function TaxOverview({ bundle, matches, fy, method }: { bundle: PortfolioBundle; matches: TaxMatch[]; fy: string; method: TaxMethod }) {
  ...
  return <>
    ...
    <TaxDisclaimer />
    <Card className="tax-report-index"><div className="card-title-row"><div><span className="section-label">REPORTS</span><h2>Open a detailed tax report</h2></div></div><div className="tax-report-links">{reportLinks.map(([label,path])=><Link to={`/deck/tax/${path}`} key={path}><FileCheck2/><span><strong>{label}</strong><small>Open report</small></span></Link>)}</div></Card>
  ...
  return <>
    ...
    <section className="mytax-section-card mytax-capital" aria-labelledby="mytax-capital-title">
      <div className="mytax-section-head">
        ...
        <Link className="mytax-section-link" to="/deck/tax/capital-gains">Go to CGT report <ArrowUpRight size={15} aria-hidden="true" /></Link>
      </div>
    ...
    <section className="mytax-section-card mytax-income-summary" aria-labelledby="mytax-income-title">
      <div className="mytax-section-head">
        ...
        <Link className="mytax-section-link" to="/deck/tax/taxable-income">Go to Taxable Income report <ArrowUpRight size={15} aria-hidden="true" /></Link>
      </div>
...
function TaxableIncome({ transactions }: { transactions: Transaction[] }) {
  ...
  return <>
    ...
    <Card className="data-card tax-field-card"><div className="card-title-row"><div><span className="section-label">SOURCE RECORDS</span><h2>Income ledger</h2></div><Badge>{rows.length} records</Badge></div>{rows.length ? <div className="table-scroll"><table><thead><tr><th>Holding</th><th>Date paid</th><th>Type</th><th>Description</th><th className="numeric">Net amount</th><th className="numeric">Franking credits</th><th className="numeric">Gross total</th></tr></thead><tbody>{rows.map((item,index)=>{const value=Math.abs(item.amount*item.fx_rate);return <HoldingNavigationRow symbol={item.symbol} key={`${item.date}-${index}`}><td><span className="asset-cell compact"><HoldingLogo symbol={item.symbol || 'CASH'} market={item.market} size={29} /><strong>{item.symbol||'Cash'}</strong></span></td><td>{date(item.date)}</td><td>{item.type}</td><td>{item.description||'Income'}</td><td className="numeric">{value === null ? 'Not supplied' : money(value)}</td><td className="numeric">Not supplied</td><td className="numeric"><strong>Not supplied</strong></td></HoldingNavigationRow>})}</tbody></table></div>:<EmptyState icon={FileCheck2} title="No income source records" description="Imported dividend, distribution and interest records appear here."/>}</Card>
  </>
}

function Valuation({ bundle }: { bundle: PortfolioBundle }) { return <HoldingTaxTable title="Holdings valuation" headers={['Holding','Name','Units','Average unit cost','Total cost','Current price','Value','Profit/loss','P/L %']} rows={bundle.holdings.map((h)=>[h.symbol,h.name||'—',number(h.quantity,4),money(h.average_cost,h.currency,2),money(h.cost_aud),money(h.current_price,h.currency,2),money(h.value_aud),money(h.unrealised_gain_aud),`${h.return_pct.toFixed(2)}%`])} markets={bundle.holdings.map(h => h.market)}/> }
function HistoricalCost({ bundle }: { bundle: PortfolioBundle }) { return <HoldingTaxTable title="Current recorded cost by holding" headers={['Holding','Quantity','Average unit cost','Recorded cost AUD']} rows={bundle.holdings.map(h => [h.symbol,number(h.quantity,4),money(h.average_cost,h.currency,2),money(h.cost_aud)])} markets={bundle.holdings.map(h => h.market)}/> }
function Unrealised({ bundle }: { bundle: PortfolioBundle }) { const gains=bundle.holdings.reduce((s,h)=>s+Math.max(0,h.unrealised_gain_aud),0);const losses=bundle.holdings.reduce((s,h)=>s+Math.min(0,h.unrealised_gain_aud),0);return <><TaxMetricStrip><TaxMetric label="Unrealised gains" value={money(gains)}/><TaxMetric label="Unrealised losses" value={money(losses)}/><TaxMetric label="Potential net gain" value={money(gains+losses)}/></TaxMetricStrip><HoldingTaxTable title="Current open positions" headers={['Symbol','Quantity','Market price','Recorded cost AUD','Market value AUD','Gain/loss AUD']} rows={bundle.holdings.map(h=>[h.symbol,number(h.quantity,4),money(h.current_price,h.currency,2),money(h.cost_aud),money(h.value_aud),money(h.unrealised_gain_aud)])} markets={bundle.holdings.map(h => h.market)}/><TaxDisclaimer /></> }

function HoldingTaxTable({ title, headers, rows, markets = [] }: { title: string; headers: string[]; rows: ReactNode[][]; markets?: Array<string | null | undefined> }) { return <Card className="data-card tax-field-card"><div className="card-title-row"><h2>{title}</h2><Badge>{rows.length} holdings</Badge></div>{rows.length?<div className="table-scroll"><table><thead><tr>{headers.map((header,index)=><th key={header} className={index>1?'numeric':''}>{header}</th>)}</tr></thead><tbody>{rows.map((row,index)=><HoldingNavigationRow symbol={String(row[0])} key={index}>{row.map((value,cell)=><td key={cell} className={cell>1?'numeric':''}>{cell===0?<span className="asset-cell compact"><HoldingLogo symbol={String(value)} market={markets[index]} size={29} /><strong>{value}</strong></span>:value}</td>)}</HoldingNavigationRow>)}</tbody></table></div>:<EmptyState icon={Scale} title="No holdings to report" description="Import or connect a portfolio to populate this report."/>}</Card> }
