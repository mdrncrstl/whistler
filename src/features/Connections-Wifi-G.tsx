...
export function Connections() {
  ...
  return (
    <>
      <PageHeader title="Connections" description="Interactive Brokers syncs automatically. Every other broker imports from a CSV export, mapped once and reviewed before anything is saved. All of it is read-only." />
      {!bundle.holdings.length && <section className="connection-onboarding" aria-label="Portfolio setup progress"><div><strong>Add your first holdings</strong><p>Choose a source below, then review the holdings Masterdeck finds. Your portfolio fills automatically after a successful sync or import, with Australian CGT records ready for tax time.</p></div><ol><li className="active"><span>1</span>Choose a source</li><li><span>2</span>Review and import</li></ol></section>}
      ...
      <section className="csv-help" data-setup-provider="csv">
        <h2>Drop in an export from any broker</h2>
        <p>Masterdeck reads the file and works out whether it is holdings or trades, which column is which, and the exchange and currency. You confirm what it found. This is a file import, not a live account connection.</p>
        <div className="csv-help-actions"><Button variant="primary" icon={CloudUpload} onClick={() => { setImportSource('CSV'); setImportBroker(null); genericFileInput.current?.click() }}>Choose CSV file</Button><a className="button" href="/templates/masterdeck-holdings.csv" download>Download holdings template</a><a className="button" href="/templates/masterdeck-trades.csv" download>Download trades template</a></div>
      </section>
      ...
      <button type="button" className={`catalog-disclosure ${catalogOpen ? 'is-open' : ''}`} aria-expanded={catalogOpen} onClick={()=>setCatalogOpen(!catalogOpen)}><span><strong>Browse supported brokers</strong><small>Only if you want to start from a name. Dropping the file above works the same way.</small></span><ChevronDown size={18} aria-hidden="true"/></button>{catalogOpen && <><div className="catalog-heading"><div><h2>Find your broker</h2><p>Interactive Brokers syncs itself and Superhero statements are read directly. Everything else imports from a CSV export, which works with any broker whether or not it is listed here.</p></div><label className="catalog-search"><Search size={15}/><input aria-label="Find a broker or exchange" value={catalogQuery} onChange={event=>setCatalogQuery(event.target.value)} placeholder="Find a broker or exchange…"/></label></div>
