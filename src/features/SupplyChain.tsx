...
export function SupplyChain({ embedded = false, embeddedSymbol }: { embedded?: boolean; embeddedSymbol?: string } = {}) {
  ...
  const choose = (item: SupplyChainCompany) => { if (!item.ticker) return; setQuery(''); setSelected(null); navigate(`/workspace/tools/supply-chain/${encodeURIComponent(item.ticker)}`) }
  ...
  if (fund) return embedded ? null : <section className="intelligence-page"><h1>Supply chain intelligence</h1><p>Company relationships do not apply to this fund. Open an individual company to explore its business relationships.</p><Link to="/workspace/tools/supply-chain/AAPL">Explore companies</Link></section>
  return <section className={`intelligence-page ${embedded ? 'is-embedded' : ''}`} id={embedded ? 'company-relationships' : undefined}>
    <header className="intelligence-heading"><div>{embedded ? <h2>Company relationships</h2> : <h1>Supply chain intelligence</h1>}<p>Understand the businesses behind this company, with sources you can inspect.</p></div>{embedded && <Link to={`/workspace/tools/supply-chain/${ticker}`}>Open explorer <ExternalLink size={13}/></Link>}</header>
