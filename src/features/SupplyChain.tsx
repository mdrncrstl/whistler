...
import { MotionExpand, MotionPopover, SlidingTabs } from '../components/ui'
...
export function SupplyChain({ embedded = false, embeddedSymbol }: { embedded?: boolean; embeddedSymbol?: string } = {}) {
  ...
  const choose = (item: SupplyChainCompany) => { if (!item.ticker) return; setQuery(''); setSelected(null); navigate(`/deck/tools/supply-chain/${encodeURIComponent(item.ticker)}`) }
  ...
  if (fund) return embedded ? null : <section className="intelligence-page"><h1>Supply chain intelligence</h1><p>Company relationships do not apply to this fund. Open an individual company to explore its business relationships.</p><Link to="/deck/tools/supply-chain/AAPL">Explore companies</Link></section>
  return <section className={`intelligence-page ${embedded ? 'is-embedded' : ''}`} id={embedded ? 'company-relationships' : undefined}>
    <header className="intelligence-heading"><div>{embedded ? <h2>Company relationships</h2> : <h1>Supply chain intelligence</h1>}<p>Understand the businesses behind this company, with sources you can inspect.</p></div>{embedded && <Link to={`/deck/tools/supply-chain/${ticker}`}>Open explorer <ExternalLink size={13}/></Link>}</header>
    {!embedded && <div className="intelligence-search"><Search size={16}/><input aria-label="Search companies" value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && candidates[0]) choose(candidates[0]); if (event.key === 'Escape') setQuery('') }} placeholder="Search company or ticker…"/><MotionPopover open={Boolean(query.trim())} className="intelligence-suggestions" ariaLabel="Company suggestions" origin="top left">{candidates.length ? candidates.map(item => <button key={item.id} onClick={() => choose(item)}><HoldingLogo symbol={companyLogoSymbol(item)} market={item.market}/><span>{item.name}</span><small>{item.ticker}</small></button>) : <p>No companies found.</p>}</MotionPopover></div>}
    <div className="intelligence-company"><HoldingLogo symbol={ticker} market={company.market} size={38}/><div><h2>{company.name}</h2><span>{ticker}{company.market ? ` · ${company.market}` : ''}</span></div><div className="intelligence-source">{loading ? 'Loading latest filing…' : network ? <><span>{network.filing.form} · {network.filing.filedAt}</span><a href={network.filing.url} target="_blank" rel="noreferrer">Read filing <ExternalLink size={12}/></a></> : <span>Filing data unavailable</span>}</div></div>
    <div className="intelligence-controls"><div className="intelligence-filters"><SlidingTabs className="intelligence-filter-tabs" options={(['all', 'supplier', 'customer', 'competitor', 'partner'] as const).map(type => ({ value: type, label: <><span className="relationship-filter-label">{type === 'all' ? 'All' : `${relationshipSingular[type]}s`}</span><span className="relationship-filter-count">{entries.filter(item => type === 'all' || item.type === type).length}</span></> }))} value={filter} onChange={(value) => { setFilter(value); setSelected(null) }} ariaLabel="Relationship filters" /></div><div className="intelligence-view"><SlidingTabs className="intelligence-view-tabs" options={[{ value: 'list', label: 'List' }, { value: 'map', label: 'Map' }]} value={view} onChange={setView} ariaLabel="Relationship view" /></div></div>
    ...
    {!visible.length ? <div className="intelligence-empty"><h3>{loading ? 'Reading company relationships…' : 'No sourced relationships available'}</h3><p>{loading ? 'The latest available annual filing is being checked.' : 'Try another company or include clearly labelled curated background. Absence from this view does not mean a relationship does not exist.'}</p></div> : view === 'map' ? <RelationshipNetwork activeCompany={company} nodes={nodes} links={links} selectedRelationshipId={selected} onSelectRelationship={setSelected} onOpenCompany={choose}/> : <div className="intelligence-list">{visible.map(item => <button key={item.relationship.id} className={selected === item.relationship.id ? 'selected' : ''} onClick={() => setSelected(selected === item.relationship.id ? null : item.relationship.id)} aria-expanded={selected === item.relationship.id}><HoldingLogo symbol={companyLogoSymbol(item.company)} market={item.company.market} size={30}/><span><strong>{item.company.name}</strong><small>{item.company.ticker || 'Private company'}</small></span><span className="intelligence-type">{relationshipSingular[item.type]}</span><span className={`intelligence-evidence ${item.relationship.sourceKind !== 'curated' ? 'verified' : ''}`}>{item.relationship.sourceKind === 'live-sec' ? 'Filing' : item.relationship.sourceKind === 'company-disclosure' ? 'Disclosure' : 'Background'}</span></button>)}</div>}
    <MotionExpand open={Boolean(selection)} className="intelligence-detail-reveal">
