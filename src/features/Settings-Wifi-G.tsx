...
function AccountSettings({ profile, name, accountName, planLabel, planDetail, onNameChange }: {
  ...
  return <>
    ...
    <div className="settings-group settings-group-compact">
      ...
      <div className="settings-status-row"><span><strong>{planLabel}</strong><small>{planDetail}</small></span><Link to="/deck/billing">Manage plan <ArrowUpRight size={14}/></Link></div>
    </div>
  </>
}
...
function DataSettings({ demo }: { demo: boolean }) {
  return <>
    <div className="settings-group settings-group-first">
      ...
      <SettingsLinkRow icon={Link2} title="Broker connections" detail="Connect, sync and monitor read-only broker feeds." to="/deck/connections" action="Manage connections"/>
      <SettingsLinkRow icon={Database} title="Imported records" detail={demo ? 'This demo uses local sample data only.' : 'Holdings and transactions remain isolated to your account.'} to="/deck/transactions" action="View transactions"/>
      <SettingsLinkRow icon={Calculator} title="Tax records" detail="Review the source transactions behind CGT calculations." to="/deck/tax" action="Open Tax reporting"/>
    </div>
...
function BillingSettings({ planLabel, planDetail, paid }: { planLabel: string; planDetail: string; paid: boolean }) {
  return <>
    ...
    <div className="settings-group settings-group-compact">
      <SettingsLinkRow icon={CreditCard} title="Plans & billing" detail="Compare plans or manage your subscription securely." to="/deck/billing" action="Open billing"/>
      <SettingsLinkRow icon={Gift} title="Referral programme" detail="Share Masterdeck and track earned account credit." to="/deck/referrals" action="View referrals"/>
    </div>
  </>
}
