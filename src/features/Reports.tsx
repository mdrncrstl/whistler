...
import { MotionExpand, SlidingTabs } from '../components/ui'
...
function Benchmark({ bundle }: { bundle: PortfolioBundle }) {
  ...
  const [proGraphMode, setProGraphMode] = useState(Boolean(bundle.profile?.settings?.proGraphMode))
  const [proStyle, setProStyle] = useState<FinanceChartStyle>('area')
  const [comparison, setComparison] = useState('benchmark')
  ...
  return <>
    ...
    <Card className="report-chart-card"><div className="card-title-row"><div><span className="section-label">PORTFOLIO VS BENCHMARK</span><h2>Recorded portfolio and benchmark values</h2></div><div className="report-chart-actions"><Badge tone={portfolioReturn >= benchmarkReturn ? 'success' : 'warning'}>{first?.benchmark_value_aud && last?.benchmark_value_aud ? `${percent(portfolioReturn - benchmarkReturn)} value growth difference` : 'Benchmark unavailable'}</Badge><FinanceProToggle enabled={proGraphMode} onChange={setProGraphMode}/></div></div><MotionExpand open={proGraphMode} className="finance-pro-reveal"><FinanceProToolbar chartStyle={proStyle} onChartStyleChange={setProStyle} comparison={comparison} onComparisonChange={setComparison} comparisonOptions={[{ id: 'benchmark', label: 'Portfolio benchmark', detail: 'Recorded benchmark history' }]} indicators={indicators} onIndicatorsChange={setIndicators} candleAvailable={false} barAvailable/></MotionExpand>{snapshots.length > 1 ? <FinanceChart points={snapshots.map(p => ({ date: p.date, value: p.value_aud, comparison: !proGraphMode || comparison === 'benchmark' ? p.benchmark_value_aud ?? undefined : undefined }))} label="Portfolio" comparisonLabel="Portfolio benchmark" chartStyle={proGraphMode ? proStyle : 'area'} indicators={proGraphMode ? indicators : []} formatValue={v => money(v, 'AUD', 2)} formatAxis={v => '$' + Intl.NumberFormat('en', { notation: 'compact' }).format(v)}/> : <EmptyState icon={Scale} title="Benchmark history is building" description="Daily snapshots are required for a like-for-like comparison."/>}<FinancePeriodSelector value={chartPeriod} onChange={setChartPeriod} ariaLabel="Benchmark chart period"/></Card>
...
function Diversification({ bundle }: { bundle: PortfolioBundle }) {
  ...
  return <>
    ...
    <div className="report-toolbar report-subtoolbar"><span>View allocation by</span><SlidingTabs className="segmented" options={[{ value: 'provider', label: 'Broker' }, { value: 'sector', label: 'Sector' }, { value: 'asset_class', label: 'Asset class' }]} value={view} onChange={(value) => setView(value as 'provider' | 'sector' | 'asset_class')} ariaLabel="Allocation grouping" /></div>
...
function Growth({ bundle }: { bundle: PortfolioBundle }) {
  ...
  const [proGraphMode, setProGraphMode] = useState(Boolean(bundle.profile?.settings?.proGraphMode))
  const [proStyle, setProStyle] = useState<FinanceChartStyle>('area')
  const [comparison, setComparison] = useState('invested')
  const [indicators, setIndicators] = useState<FinanceIndicatorId[]>([])
  ...
  return <>
    ...
    <Card className="report-chart-card"><div className="card-title-row"><div><span className="section-label">GROWTH HISTORY</span><h2>Portfolio value vs contributed capital</h2></div><div className="report-chart-actions"><SlidingTabs className="segmented" options={[{ value: 'amount', label: 'Amount' }, { value: 'percent', label: 'Percent' }, ...(!proGraphMode ? [{ value: 'line', label: 'Line' }, { value: 'bar', label: 'Bar' }] : [])]} value={proGraphMode ? displayMode : chartMode} onChange={(value) => { if (value === 'amount' || value === 'percent') setDisplayMode(value); else if (value === 'line' || value === 'bar') setChartMode(value) }} ariaLabel="Growth chart display" /><FinanceProToggle enabled={proGraphMode} onChange={setProGraphMode}/></div></div><MotionExpand open={proGraphMode} className="finance-pro-reveal"><FinanceProToolbar chartStyle={proStyle} onChartStyleChange={setProStyle} comparison={comparison} onComparisonChange={setComparison} comparisonOptions={[{ id: 'invested', label: 'Contributed capital', detail: 'Recorded deposits and withdrawals' }]} indicators={indicators} onIndicatorsChange={setIndicators} candleAvailable={false} barAvailable/></MotionExpand>{chartSnapshots.length > 1 ? <FinanceChart points={chartData.map(p => ({ date: p.date, value: p.value, comparison: !proGraphMode || comparison === 'invested' ? p.invested : undefined, measurementValue: p.value_aud }))} label="Portfolio" comparisonLabel="Contributed capital" chartStyle={proGraphMode ? proStyle : chartMode === 'bar' ? 'bar' : 'area'} indicators={proGraphMode ? indicators : []} formatValue={formatGrowthValue} bars={!proGraphMode && chartMode === 'bar'}/> : <EmptyState icon={TrendingUp} title="Growth history is building" description="Portfolio snapshots will appear here as prices sync."/>}<FinancePeriodSelector value={chartPeriod} onChange={setChartPeriod} ariaLabel="Growth chart period"/></Card>
...
function IncomeBreakdown({ bundle }: { bundle: PortfolioBundle }) {
  ...
  return <>
    ...
    <Card className="data-card report-detail-card"><div className="card-title-row"><h2>Holdings breakdown</h2><Badge>{income.length} payments</Badge></div>{income.length ? <div className="table-scroll"><table><thead><tr><th>Symbol</th><th>Date paid</th><th>Type</th><th>Description</th><th className="numeric">Total</th></tr></thead><tbody>{income.map((item,index) => <HoldingNavigationRow symbol={item.symbol} key={`${item.date}-${item.symbol}-${index}`}><td><span className="asset-cell compact"><HoldingLogo symbol={item.symbol || 'CASH'} market={item.market} size={29} /><span><strong>{item.symbol || 'Cash'}</strong><small>{item.account_name}</small></span></span></td><td>{date(item.date)}</td><td><Badge tone="success">{item.type}</Badge></td><td>{item.description || 'Income payment'}</td><td className="numeric positive">{money(item.aud)}</td></HoldingNavigationRow>)}</tbody></table></div> : <EmptyState icon={CalendarDays} title="No income events" description="Imported payment records appear in this ledger."/>}</Card>
...
function IncomeCalendar({ bundle }: { bundle: PortfolioBundle }) {
  ...
  return <>
    ...
    <Card className="data-card report-detail-card"><div className="card-title-row"><div><span className="section-label">SCHEDULE</span><h2>Upcoming payments</h2></div><span>Estimated from prior recorded payments</span></div>{projected.length ? <div className="table-scroll"><table><thead><tr><th>Holding</th><th>Estimated ex-date</th><th>Payment date</th><th>Status</th><th className="numeric">Net amount</th></tr></thead><tbody>{projected.map((item) => <HoldingNavigationRow symbol={item.symbol} key={item.id}><td><span className="asset-cell compact"><HoldingLogo symbol={item.symbol || 'CASH'} market={item.market} size={29} /><span><strong>{item.symbol || 'Cash'}</strong><small>{item.description}</small></span></span></td><td>{date(item.date)}</td><td>{date(item.date)}</td><td><Badge tone="warning">Estimated</Badge></td><td className="numeric positive">{money(item.aud)}</td></HoldingNavigationRow>)}</tbody></table></div> : <EmptyState icon={CalendarDays} title="No upcoming payments to estimate" description="A schedule appears once income history has been imported."/>}</Card>
