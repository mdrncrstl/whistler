...
function brokerLabel(provider: string) {
  if (provider === 'ibkr') return 'IBKR'
  if (provider === 'superhero') return 'Superhero'
  if (provider === 'snaptrade') return 'Connected broker'
  return provider
}
    ...
    <>
      ...
      <Card className="data-card">
        <div className="data-toolbar">
          ...
          <div className="toolbar-filters"><ListFilter size={16} /><Select value={type} onChange={(event) => { setType(event.target.value); setPage(1) }} aria-label="Filter transaction type"><option value="all">All activity</option>{transactionTypes.map((item) => <option key={item}>{item}</option>)}</Select><Select value={provider} onChange={(event) => { setProvider(event.target.value); setPage(1) }} aria-label="Filter transaction broker"><option value="all">All brokers</option><option value="ibkr">IBKR</option><option value="superhero">Superhero</option><option value="snaptrade">Connected broker</option></Select></div>
        </div>
        {rows.length ? <div className="table-scroll"><table className="transactions-table"><thead><tr><th>Date</th><th>Activity</th><th>Asset</th><th>Account</th><th className="numeric">Quantity</th><th className="numeric">Price</th><th className="numeric">Amount</th><th className="numeric">Fees</th></tr></thead><tbody>{rows.map((item) => { const typeLabel = String(item.type).toUpperCase(); return <HoldingNavigationRow symbol={item.symbol} key={`${item.provider}-${item.provider_external_id}`}><td><strong>{date(item.date, { day: '2-digit', month: 'short', year: 'numeric' })}</strong></td><td><Badge tone={toneForType(typeLabel)}>{typeLabel}</Badge></td><td><span className="asset-cell simple"><HoldingLogo symbol={item.symbol || 'CASH'} market={item.market} size={29} /><span><strong>{item.symbol || 'Cash'}</strong><small>{item.description || typeLabel}</small></span></span></td><td><span>{brokerLabel(item.provider)}</span><small className="account-subline">{item.account_name || 'Portfolio'}</small></td><td className="numeric">{item.quantity ? number(item.quantity, 4) : '–'}</td><td className="numeric">{item.price ? money(item.price, item.currency, 2) : '–'}</td><td className={`numeric private-value ${item.amount >= 0 ? 'positive-soft' : ''}`}><strong>{money(item.amount * (item.fx_rate || 1), 'AUD', 2)}</strong><small>{item.currency !== 'AUD' ? `${money(item.amount, item.currency, 2)} ${item.currency}` : 'AUD'}</small></td><td className="numeric private-value">{item.fees ? money(item.fees * (item.fx_rate || 1), 'AUD', 2) : '–'}</td></HoldingNavigationRow> })}</tbody></table></div> : <EmptyState icon={bundle.transactions.length ? SearchX : TableProperties} title={bundle.transactions.length ? 'No ledger rows match' : 'No transactions imported'} description={bundle.transactions.length ? 'Try a broader search or remove a filter.' : 'Import records from a CSV, PDF statement or supported direct feed to create a deduplicated activity ledger.'} />}
