...
import { Badge, Button, Card, EmptyState, IconButton, MotionExpand, PageHeader, SlidingTabs } from '../components/ui'
...
export function Tools() {
  ...
  if (tool === 'inbox') return <Navigate to="/deck/connections" replace />
...
function DeckAI() {
  ...
  const researchComparisonOptions = useMemo(() => {
    const popular = [
      { id: 'AMZN', label: 'AMZN', detail: 'Amazon.com Inc · NASDAQ (US)', market: 'NASDAQ' },
      { id: 'MSFT', label: 'MSFT', detail: 'Microsoft Corp. · NASDAQ (US)', market: 'NASDAQ' },
      { id: 'GOOG', label: 'GOOG', detail: 'Alphabet Inc Class C · NASDAQ (US)', market: 'NASDAQ' },
      { id: 'NVDA', label: 'NVDA', detail: 'NVIDIA Corp. · NASDAQ (US)', market: 'NASDAQ' },
    ]
    const options = new Map(popular.map(item => [item.id, item]))
    bundle.holdings.forEach(holding => options.set(holding.symbol.toUpperCase(), { id: holding.symbol.toUpperCase(), label: holding.symbol.toUpperCase(), detail: `${holding.name || holding.symbol} · ${holding.market || 'Market unavailable'}`, market: holding.market || '' }))
  ...
  return <div className="ai-workspace ai-engine">
    ...
    <div className="ai-conversation" ref={conversationRef}>
      {!messages.length ? <div className="ai-engine-welcome"><ThinkingOrb state="breathing" size={64} paused={!!reducedMotion} aria-hidden="true"/><span className="section-label">A LITTLE MORE CLARITY</span><h2>Your next question.<br/><em>A clearer answer.</em></h2><p>Explore your investments or look up a company by name or ticker. Start with a question below.</p><SlidingTabs className="ai-question-tabs" options={(Object.keys(questionGroups) as (keyof typeof questionGroups)[]).map(group => ({ value: group, label: group }))} value={category} onChange={setCategory} ariaLabel="Question categories" /><AnimatePresence mode="wait" initial={false}><motion.div key={category} className="ai-suggestion-grid" initial={reducedMotion ? false : { opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? undefined : { opacity: 0, y: -6 }} transition={{ duration: reducedMotion ? 0.01 : 0.18, ease: [0.23, 1, 0.32, 1] }}>{suggestions.map((item, index) => <button key={item} onClick={() => send(item)}><small>0{index + 1}</small><span>{item}</span><ArrowUpRight size={17}/></button>)}</motion.div></AnimatePresence></div> : <div className="ai-messages" role="log" aria-label="Portfolio conversation" aria-live="polite">{messages.map((message,index) => <div className={`ai-message ${message.role}`} key={index}><span className={`ai-message-avatar ${message.role === 'user' ? 'is-user' : ''}`}>{message.role === 'assistant' ? <ThinkingOrb state="shaping" size={20} paused aria-hidden="true"/> : null}</span><article>{message.answer && <div className="ai-answer-heading">{message.answer.symbol && <HoldingLogo symbol={message.answer.symbol} market={message.answer.exchange} href={companyUrlForSymbol(message.answer.symbol, message.answer.exchange)} size={34}/>}<h2>{message.answer.title}</h2></div>}<p>{message.text}</p>{message.answer?.metrics && <div className="ai-answer-metrics">{message.answer.metrics.map((metric, i) => <div key={`${metric.label}-${i}`}><small>{metric.label}</small><strong>{metric.value}</strong></div>)}</div>}{message.answer?.points?.length ? <StockResearchChart points={message.answer.points} symbol={message.answer.symbol || message.answer.title} currency={message.answer.currency || 'AUD'} market={message.answer.exchange} comparisonOptions={researchComparisonOptions} proGraphMode={Boolean(bundle.profile?.settings?.proGraphMode)}/> : null}{message.answer?.source && <a className="ai-source-link" href={message.answer.source.url} target="_blank" rel="noreferrer">{message.answer.source.label} · {message.answer.source.asOf ? new Date(message.answer.source.asOf).toLocaleString('en-AU') : 'Quote time unavailable'} · May be delayed<ArrowUpRight size={13}/></a>}{!!message.answer?.alternatives?.length && <div className="ai-alternatives"><small>Other matching listings</small>{message.answer.alternatives.map(item => <button key={item.symbol} disabled={preparing} onClick={() => send(`Look up ${item.symbol}`)}>{item.symbol} · {item.name}<ArrowUpRight size={12}/></button>)}</div>}{message.answer?.href && <Link to={message.answer.href}>{message.answer.link}<ArrowUpRight size={14}/></Link>}</article></div>)}<div role="status" className="ai-preparing" aria-live="polite">{preparing && <><ThinkingOrb state={researching ? 'searching' : 'solving'} size={20} paused={!!reducedMotion} aria-hidden="true"/><div><strong>{researching ? 'Looking up that stock' : 'Preparing your answer'}</strong><span>{researching ? 'Checking current market data' : 'Calculating from your portfolio records'}</span></div></>}</div><div className="ai-conversation-end" aria-hidden="true"/></div>}
    </div>
