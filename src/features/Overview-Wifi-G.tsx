...
function HoldingRow({ item, income, visibleColumns, menuOpen, onMenu }: { item: Position; income: number; visibleColumns: ColumnKey[]; menuOpen: boolean; onMenu: () => void }) {
  ...
  return <HoldingNavigationRow symbol={item.symbol}>
    <td className="portfolio-symbol"><button className="holding-link" onClick={() => navigate(`/workspace/holdings/${encodeURIComponent(item.symbol)}`)}><HoldingLogo symbol={item.symbol}/><span><strong>{item.symbol}</strong><small>{item.name || item.account_name}</small></span></button></td>
...
export function Overview() {
  ...
  const [valueMode, setValueMode] = useState<'Amount' | 'Percent'>('Amount'); const [chartType, setChartType] = useState<'Line' | 'Bar'>('Line'); const [chartPeriod, setChartPeriod] = useState<FinancePeriod>('MAX')
  const [proGraphMode, setProGraphMode] = useState(Boolean(bundle.profile?.settings?.proGraphMode)); const [proStyle, setProStyle] = useState<FinanceChartStyle>('area'); const [comparison, setComparison] = useState('none'); const [indicators, setIndicators] = useState<FinanceIndicatorId[]>([])
  ...
  useEffect(() => {
    ...
    const closeOutside = (event: PointerEvent) => {
      if (pageRef.current?.contains(event.target as Node)) return
      setRangeOpen(false)
      setFilterOpen(false)
  ...
  const metrics = [
    ...
    { id: 'capital' as const, label: 'Capital Gain', value: summary.capitalGain, percent: summary.cost ? summary.capitalGain / summary.cost * 100 : 0, description: 'Unrealised price movement, with the currency component separated when the recorded cost basis allows it.' },
    { id: 'income' as const, label: 'Income Return', value: summary.income, percent: summary.cost ? summary.income / summary.cost * 100 : 0, description: 'Recorded dividends, distributions and interest, converted to AUD using the imported FX rates. The percentage is calculated against the recorded cost base.' },
  ...
  return <div className="portfolio-page" ref={pageRef}>
    ...
    <div className="portfolio-metrics" role="tablist" aria-label="Portfolio return metric">{metrics.map((metric) => <MetricSurface key={metric.id} label={metric.label} exact={money(metric.value, 'AUD', 2)} description={metric.description} selected={activeMetric === metric.id} onSelect={() => setActiveMetric(metric.id)}><strong><PrivateMoney value={metric.value} digits={2}/></strong><small className={metric.percent > 0 ? 'positive' : metric.percent < 0 ? 'negative' : ''}>{displayPercent(metric.percent)} <em>total</em></small></MetricSurface>)}</div>
    ...
    <section className="portfolio-chart-section">
      ...
      <div className="chart-mode-row"><div><button className={valueMode === 'Amount' ? 'active' : ''} onClick={() => setValueMode('Amount')}>Amount</button><button className={valueMode === 'Percent' ? 'active' : ''} onClick={() => { setValueMode('Percent'); setChartType('Line') }}>Percent</button></div>{!proGraphMode && valueMode === 'Amount' && <div><button className={chartType === 'Line' ? 'active' : ''} onClick={() => setChartType('Line')}>Line</button><button className={chartType === 'Bar' ? 'active' : ''} onClick={() => setChartType('Bar')}>Bar</button></div>}</div>
      {proGraphMode && <FinanceProToolbar chartStyle={proStyle} onChartStyleChange={setProStyle} comparison={comparison} onComparisonChange={setComparison} comparisonOptions={activeMetric === 'portfolio' ? [{ id: 'benchmark', label: 'Portfolio benchmark', detail: 'Recorded benchmark history' }] : []} indicators={indicators} onIndicatorsChange={setIndicators} candleAvailable={false} barAvailable/>}
