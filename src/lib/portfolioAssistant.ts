...
export function portfolioAnswer(question: string, bundle: PortfolioBundle): PortfolioAnswer {
  ...
  if (/last |this (month|year|week)|yesterday|\bytd\b|\b20\d{2}\b/.test(q)) return result('Choose a reporting period', 'This chat summarises all loaded records. Use the report’s date controls for a specific period so deposits, sales and dates are handled explicitly.', '/workspace/reports/performance', 'Open performance report')
  if (!bundle.holdings.length && !bundle.transactions.length && !bundle.cash.length) return result('Start with your records', 'Connect an account or import a statement to calculate your portfolio. Nothing has been estimated or filled with example values.', '/workspace/connections', 'Add investment records')
  if (/tax|cgt|frank/.test(q)) return result('Review your tax records', 'Open-position gains are not realised capital gains. The tax report matches transaction parcels and flags missing records; review those before using an estimate in a return.', '/workspace/tax', 'Open tax overview')
  if (/unrealised gains/.test(q)) return result('Unrealised gains', 'The difference between the current value and cost of investments you still hold. It changes with prices and exchange rates. It is not the realised gain from a sale.', '/workspace/reports/performance', 'Review performance')
  if (/import|connect/.test(q)) return result('Bring your investments together', 'Open Connections to choose a supported broker, upload a statement or add records manually. Available import methods depend on the provider.', '/workspace/connections', 'Add investment records')
  ...
  if (/fees/.test(q)) return result('Fees on record', `${money(bundle.transactions.reduce((total, item) => total + Math.abs(item.fees || 0) * (item.fx_rate || 1), 0))} in recorded transaction fees, converted to AUD using each record’s exchange rate. This excludes costs not supplied in your records.`, '/workspace/transactions', 'Review transactions')
  ...
  if (/currency exposure/.test(q)) {
    ...
    return { ...result('Currency exposure', 'Holdings grouped by quote currency, valued in AUD. This does not measure the underlying currency exposure of a hedged fund.', '/workspace/reports/diversification', 'Explore diversification'), metrics: [...currencies].sort((a,b)=>b[1]-a[1]).map(([label,value])=>({label,value:money(value)})) }
  }
  if (/concentrat|allocat|exposure|diversif|largest/.test(q)) {
    const top = allocationBy(bundle.holdings, 'sector').slice(0, 3)
    return result('Where your exposure sits', top.length ? `${top.map(item => `${item.name}: ${percent(item.percentage).replace(/^\+/, '')}`).join('; ')}. Percentages use the current AUD value of holdings and exclude cash. Missing sectors are grouped as Other.` : 'No holdings are available to calculate sector exposure.', '/workspace/reports/diversification', 'Explore diversification')
  }
  ...
  if (/transaction|trade|record/.test(q)) return result('Your transaction history', `${bundle.transactions.length} transactions are loaded. Open the ledger to filter by date, account, type and ticker.`, '/workspace/transactions', 'Open transactions')
