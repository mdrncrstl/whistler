...
const defaultAppOrigin = 'https://masterdeck.app'
...
function configuredAppOrigin() {
  return Deno.env.get('APP_ORIGIN') || defaultAppOrigin
}

export function corsHeadersFor(origin?: string | null) {
  const candidate = origin && allowedAppOrigins.has(origin) ? origin : configuredAppOrigin()
  return {
...
export const json = (body: unknown, status = 200, origin?: string | null) => new Response(JSON.stringify(body), { status, headers: { ...corsHeadersFor(origin), 'Content-Type': 'application/json' } })
...
export function safeReturnOrigin(value: unknown) {
  ...
  try {
    ...
    if (candidate.origin === configured || allowedAppOrigins.has(candidate.origin)) return candidate.origin
